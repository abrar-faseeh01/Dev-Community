...
- The feed has no search, filtering, or alternative sort yet (Day 14). `commentCount` is accurate and now rendered, in the comments section's "Comments (N)" heading.
...
- Not done yet: the manual pass with a throttled and a blocked network (the plan's checkpoint 7). The "done when" behaviours are covered by the automated tests above, but I have not watched them in a browser.
