...
I'm building a Developer Community Platform where members can authenticate, maintain a developer profile, and (eventually) publish posts, comment, react, search, and browse ranked content. This repo covers the platform through Day 5 of my 20-day build plan: project foundations, authentication with role-based access, and a full developer profile API with admin oversight.
...
- `backend/` — NestJS API (`src/<feature>/` modules: `auth`, `users`, `profiles`, `audit`, `notifications`, `health`; shared code in `src/common/`).
