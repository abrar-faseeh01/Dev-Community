...
import { getPostChanges } from "@/features/posts/utils/changes";
import {
  describeLoadError,
  hasStatus,
  isPostNotFound,
...
import { getPostActor, type PostActor } from "@/features/posts/utils/permissions";
import { useUpdatePost } from "@/features/posts/mutations/post-mutations";
import { forgetPost } from "@/features/posts/utils/post-cache-updates";
...
export function EditPostView({ id }: { id: string }) {
  ...
  // See post-page-view.tsx: a post found to be gone can only be dropped
...
function EditPostForm({ post, actor, onReload, onGone }: EditPostFormProps) {
  ...
  const updateMutation = useUpdatePost(post.id, {
    onSuccess: (updated) => router.push(ROUTES.post(updated.id)),
    onError: (error) => {
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <PostForm
        ...
        cancelHref={ROUTES.post(post.id)}
        requireChange
